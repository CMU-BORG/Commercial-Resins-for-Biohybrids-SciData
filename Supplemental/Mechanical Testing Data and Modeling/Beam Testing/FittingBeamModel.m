...
% resins being tested
resins = ["Phrozen","Liqcreate"];
samp_inds = [1,2,3,4;2,3,4,5];

% Young's moduli from tensile tests
Youngs_paper = [384,709];
err_Youngs_paper = [37;50];

% Young's moduli reported by the companies
Youngs_company = [2640.5,2000];
err_Youngs_company = [207.5,0];
...
%% Looping over all of the samples
for j=1:2
    % for all resins
    resin = resins(j);
    fprintf("Resin: "+resin+"\n")
    ...
    for i=1:size(samp_inds,2)
        ...
        % determine if the sample is from the non-soaked (control) or
        % soaked group
        if i<=2
            state = "control";
        else
        ...
        folder = "Test Beam Images\" + resin+" Beams\Sample "+sample+"\";
...
%% Plotting the Summary of Results
...
g = 9.81;
[x1_,y1_] = BeamModel(E,x0,y0,theta0,params,-m1*g,S_data);
[x2_,y2_] = BeamModel(E,x0,y0,theta0,params,-m2*g,S_data);
